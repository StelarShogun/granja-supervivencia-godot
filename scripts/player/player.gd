...
func _physics_process(delta: float) -> void:
	...
	if manager != null and bool(manager.get("game_over")):
		...
		velocity.x = 0.0
		velocity.z = 0.0
		AudioManager.set_footsteps_active(false)
...
func _update_footsteps(input_dir: Vector2) -> void:
	...
	var is_running := (
		...
		and not is_crouching
		and horizontal_speed > walk_speed * 0.7
	)
	AudioManager.set_footsteps_active(is_moving, is_running)
